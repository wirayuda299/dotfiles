...
---
--- Neovim does not have built-in support for the bicep filetype which is required for   to automatically launch the language server.
---
...
--- ```
---
--- **By default, bicep language server does not have a `cmd` set.** This is because nvim-  does not make assumptions about your path. You must add the following to your init.vim or init.lua to set `cmd` to the absolute path ($HOME and ~ are not expanded) of the unzipped run script or binary.
---
--- ```lua
